...
3、语义化更好的内容标签(header,nav,footer ,aside [əˈsaɪd], article [ˈɑːtɪkl] , section[sekʃn]) 

4、音频 ,视频(audio [ˈɔːdiəʊ], video[ˈvɪdiəʊ] ) 如果浏览器不支持自动播放怎么办? 在属性中添加 autoplay(谷歌浏览器不支持音频自动播放，但是视频支持静音自动播放) 
...
7、 本地离线存储 localStorage[ˈləʊkl ˈstɔːrɪdʒ]  用于长久保存整个网站的数据，保存的数据没有过期时间， 

直到手动去删除 

8、 sessionStorage  [ˈseʃn ˈstɔːrɪdʒ]该数据对象临时保存同一窗口(或标签页)的数据，在关闭窗口或标签页 

之后将会删除这些数据。 

9、 表单控件 calendar[ˈkælɪndə(r)] , date , time , email , url , search , tel , file , number 

10、新的技术 webworker[web ˈwɜːkə(r)] , websocket [webˈsɒkɪt] , Geolocation [ji ao le tei shen]

**CSS3 新特性** 

1、颜色: 新增 RGBA , HSLA [a to fei li ya]模式 

2、文字阴影(text-shadow[ˈʃædəʊ]) 

3、边框: 圆角(border-radius) 边框阴影 : box [bɒks]-shadow 

4、盒子模型: box-sizing  [ˈsaɪzɪŋ]

5、背景:background-size background-origin [ˈɒrɪdʒɪn]  background-clip [klɪp] 

6、渐变: linear-gradient , radial-gradient 

7、过渡 : transition [trænˈzɪʃn] 可实现属性的渐变 
...
<head>
  ...
  <style>
    ...
    .child {
      ...
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);[trænzˈleɪt] 
    }
  </style>
</head>
...
1、typeof
2、instanceof
3、Object.prototype[ˈprəʊtətaɪp].toString.call({})  // 同上结果，加上call也ok
//通用方法  检测类型
...
深拷贝
(1) 利用JSON.parse[pɑːz]  和JSON.stringfy    jQuery的 `extend`[ɪkˈstend] 
var obj1 = {
  name: 'li',
  hobit: ['摄影', '羽毛球']
}
...
2、调用函数时，应该提供的参数没有提供，该参数等于 undefined o

3、对象没有赋值的属性，该属性的值为 undefined 

4、函数没有返回值时，默认返回 undefi
...
```js
{}.constructor [kənˈstrʌktə(r)] //返回 object 
...
arr3.flat(Infinity)[ɪnˈfɪnəti] ; // [1, 2, 3, 4, 5, 6]
```
...
#### 20、dom 节点的 Attribute [əˈtrɪbjuːt ,]和 Property 有何区别？

**1、什么是 Property [ˈprɑːpərti] ** 
...
#### 22、如何最小化重绘(repaint[rei pei te])和回流(reflow[ˈriːfləʊ])

重绘:当元素的一部分属性发生改变，如外观、背景、颜色等不会引起布局变化，只需要浏览器根据元素的新属性重新绘制，使元素呈现新的外观叫做重绘。 

重排（回流）:当 render [ˈrendə(r)] 树中的一部分或者全部因为大小边距等问题发生改变而需要 DOM 树重新计算的过程 。
...
2、需要创建多个 DOM 节点时，使用 DocumentFragment [ˈfræɡmənt ]  创建完后一次性的加入 document缓存 Layout 属性值，如：var left = elem.offsetLeft; 这样，多次使用 left 只产生一次回流 。
...
宏任务：包括整体代码 script，setTimeout，setInterval  [set ˈɪntəvl] 
...
- 第三次握手：客户端收到 `SYN + ACK` 报文，向服务器发送确认包，客户端进入 `ESTABLISHED`  [ɪˈstæblɪʃt] 状态。待服务器收到客户端发送的 `ACK` 包也会进入 `ESTABLISHED` 状态，完成三次握手。
...
三个状态：pending [ˈpendɪŋ] 、fulfilled[fʊlˈfɪld] 、reject 

两个过程：padding -> fulfilled、padding -> rejected 

**Promise 什么时候会进入 catch** 

当 pending 为 rejected [rɪˈdʒektɪd]  时，会进入 catch
...
Webpack 是一个打包模块化 javascript 的工具，在 Webpack 里一切文件皆模块，通过loader [ˈləʊdə(r)] 转换文件，通过 plugin 注入钩子，最后输出由多个模块组合成的文件，Webpack 专注构建模块化项目，Webpack 可以看做是模块打包机：它做的事情是，分析你的项目结构，找到 JavaScript模块以及其它的一些浏览器不能直接运行的拓展语言（Scss，TypeScript 等），并将其打包为合适的格式以供浏览器使用。
...
能力扩展。通过`webpack`的`Plugin[pu lu gai]`机制，我们在实现模块化打包和编译兼容的基础上，可以进一步实现诸如按需加载，代码压缩等一系列功能，帮助我们进一步提高自动化程度，工程效率以及打包输出的质量。`
...
**第二步：**compile [kəmˈpaɪl]  解析模板指令，将模板中的变量替换成数据，然后初始化渲染页面视图，并将每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据有变动，收到通知，更新视图 
...
compile [kəmˈpaɪl] 解析模板指令，将模板中的变量替换成数据，然后初始化渲染页面视图，并将每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据有变动，收到通知，更新视图
...
1、 项目使用 keep-alive [kiːp əˈlaɪv] 时，可搭配组件 name 进行缓存过滤 
...
2、$parent  [ˈpeərənt] 和 $children 获取当前组件的父组件和当前组件的子组件。
...
- 一、是否占据空间。
  display:none，隐藏之后不占位置；visibility[ˌvɪzəˈbɪləti] :hidden、opacity:0，[əʊˈpæsəti] 隐藏后任然占据位置。
